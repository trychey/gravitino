...
public class PaimonPropertiesConverter implements PropertiesConverter {
    ...
    public Map<String, String> toFlinkCatalogProperties(Map<String, String> gravitinoProperties) {
        ...
        String backendType = flinkCatalogProperties.get(GravitinoPaimonCatalogFactoryOptions.backendType.key());
        if (PaimonCatalogBackend.JDBC.name().equalsIgnoreCase(backendType) ){
            ...
            flinkCatalogProperties.put(CatalogOptions.URI.key(), gravitinoProperties.get(PaimonConfig.CATALOG_URI.getKey()));
            flinkCatalogProperties.put("jdbc.user", gravitinoProperties.get(PaimonConfig.CATALOG_JDBC_USER.getKey()));
            flinkCatalogProperties.put("jdbc.password",gravitinoProperties.get(PaimonConfig.CATALOG_JDBC_PASSWORD.getKey()));
        }else if(PaimonCatalogBackend.HIVE.name().equalsIgnoreCase(backendType)) {
            throw new UnsupportedOperationException("The Gravitino Connector does not currently support creating a Paimon Catalog that uses Hive Metastore.");
        }
